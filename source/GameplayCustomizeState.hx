...
class GameplayCustomizeState extends MusicBeatState
{
    ...
    var background:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('stageback','preload'));
    var curt:FlxSprite = new FlxSprite(-500, -300).loadGraphic(Paths.image('stagecurtains','preload'));
    var front:FlxSprite = new FlxSprite(-650, 600).loadGraphic(Paths.image('stagefront','preload'));

    var sick:FlxSprite = new FlxSprite().loadGraphic(Paths.image('sick','preload'));
	...
	private function generateStaticArrows(player:Int):Void
        {
            for (i in 0...4)
            {
                ...
                babyArrow.frames = Paths.getSparrowAtlas('NOTE_assets', 'preload');
