	...
	public function new(x:Float, y:Float, ?character:String = "bf", ?isPlayer:Bool = false)
	{
		...
		switch (curCharacter)
		{
			...
			case 'bf-holding-gf-dead':
				...
				var tex = FlxAtlasFrames.fromSparrow('assets/images/bfHoldingGF-DEAD.png', 'assets/images/bfAndGf.xml');
				frames = tex;
			...
			default:
				...
				if (EngineFunctions.exists(path + curCharacter.toLowerCase()))
				{
					...
					if (containsJson)
					{
						...
						{
							animation.addByPrefix(jsonParse.animations[i].name, jsonParse.animations[i].anim, jsonParse.animations[i].fps, jsonParse.animations[i].loop);
						}

						for (i in 0...jsonParse.animations.length)
						{
							addOffset(jsonParse.animations[i].name, jsonParse.animations[i].offsets[0], jsonParse.animations[i].offsets[1]);
						}

						playAnim(jsonParse.playAnim);

						flipX = jsonParse.flipX;
					}
				}
				else
				{
					trace("Warning: The custom character you loaded either doesn't exist or it's corrupted. The character has been replaced with Boyfriend");
					var tex = Paths.getSparrowAtlas('BOYFRIEND');
					frames = tex;
					animation.addByPrefix('idle', 'BF idle dance', 24, false);
					animation.addByPrefix('singUP', 'BF NOTE UP0', 24, false);
				...
				#end
				#if html5
				trace("Custom character not supported in html5");
				#end
		}
