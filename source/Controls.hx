...
class Controls extends FlxActionSet
{
	...
	public function setKeyboardScheme(scheme:KeyboardScheme, reset = true)
	{
		...
		{
			case Solo:
				inline bindKeys(Control.UP, [FlxKey.fromString(Main.UPBind)]);
				inline bindKeys(Control.DOWN, [FlxKey.fromString(Main.DOWNBind)]);
				inline bindKeys(Control.LEFT, [FlxKey.fromString(Main.LEFTBind)]);
				inline bindKeys(Control.RIGHT, [FlxKey.fromString(Main.RIGHTBind)]);
	...
	public function loadKeybinds(?reset:Bool)
	{
		if (reset)
		{
			removeKeyboard();
		...
		#if html5
		inline bindKeys(Control.UP, [FlxKey.fromString(FlxG.save.data.UPBind)]);
		inline bindKeys(Control.DOWN, [FlxKey.fromString(FlxG.save.data.DOWNBind)]);
		inline bindKeys(Control.LEFT, [FlxKey.fromString(FlxG.save.data.LEFTBind)]);
		inline bindKeys(Control.RIGHT, [FlxKey.fromString(FlxG.save.data.RIGHTBind)]);
		#else
		inline bindKeys(Control.UP, [FlxKey.fromString(EngineFunctions.getKey("UP"))]);
		inline bindKeys(Control.DOWN, [FlxKey.fromString(EngineFunctions.getKey("DOWN"))]);
		inline bindKeys(Control.LEFT, [FlxKey.fromString(EngineFunctions.getKey("LEFT"))]);
		inline bindKeys(Control.RIGHT, [FlxKey.fromString(EngineFunctions.getKey("RIGHT"))]);
		#end
		inline bindKeys(Control.ACCEPT, [Z, SPACE, ENTER]);
		inline bindKeys(Control.BACK, [BACKSPACE, ESCAPE]);
		inline bindKeys(Control.PAUSE, [P, ENTER, ESCAPE]);
